...
function sed_changes() {
      ...
      if [ "$2" == "OSD_ON_DEVICE" ]; then
       ...
       if [ "$2" == "OSD_ON_PVC" ]; then
           sed -i "s/allowMultiplePerNode: false/allowMultiplePerNode: true/" ./cluster/examples/kubernetes/ceph/cluster-on-pvc.yaml
...
while [[ $(kubectl get pods -l app=rook-ceph-operator -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}' -n rook-ceph) != "True" ]]; do echo "waiting for rook operator pod" && sleep 1; done
